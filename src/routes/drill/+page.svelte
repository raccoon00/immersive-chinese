...
<script lang="ts">
	...
	import { setHanziStrokeDataContext } from '$lib/hanzi/context';
	import { getMissingStrokeDataChars } from '$lib/hanzi/loadHanziData';
	...
	const hanziStrokeData = $derived(data.hanziStrokeData);

	setHanziStrokeDataContext(() => data.hanziStrokeData);

	const drillId = $derived(drill.id);
	const cards = $derived(drill.cards);
	const cardIds = $derived(cards.map((card) => card.id));
	const uniqueChars = $derived(
		[...new Set(cards.flatMap((card) => Array.from(card.hanzi)))].sort()
	);
	const uniqueHanziCount = $derived(uniqueChars.length);
	const missingStrokeDataChars = $derived(getMissingStrokeDataChars(uniqueChars, hanziStrokeData));
	...
	function isTrainableCharacterResult(characterResult: CharacterValidationResult): boolean {
		return Boolean(characterResult.targetChar && hanziStrokeData[characterResult.targetChar]);
	}
	...
	async function finishAttempt(): Promise<void> {
		...
		try {
			...
			void logEvent('client_error_reported', {
				...
				component: 'drill/+page.svelte:finishAttempt',
				attemptId,
				drillId,
				cardId: currentCard.id
			});
...
<svelte:head>
	<title>Hanzi Practice · Drill {drillId}</title>
	<meta
		name="description"
	...
	>
		<header class="space-y-2 sm:space-y-3">
			...
			<p class="text-sm text-zinc-600">
				Translation-prompted recall with drawing validation, live logs, and training feedback.
			</p>
		</header>
		...
		<section class="grid gap-3 md:grid-cols-2 sm:gap-4">
			<div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200 sm:p-5">
				...
				<p class="mt-3 text-sm text-zinc-600">
					Server validation still runs in production and now receives drill-aware card data in the
					request body.
				</p>
				{#if lastLogError}
			...
			<div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200 sm:p-5">
				...
				<p class="mt-1 text-sm text-zinc-500">Queue seed: {queueSeed || 'pending'}</p>
